...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        @if($updateMode)
            @include('livewire.holidays.create')
        @elseif($createMode)
            @include('livewire.holidays.create')
        @else
            <table class="table-fixed w-full">
                            ...
                            <thead>
                                <tr class="bg-gray-100">
                                    ...
                                    <th class="px-4 py-2">Holiday Description</th>
                            ...
                            @php $no = 1; @endphp
                                ...
                                <td class="border px-4 py-2">{{ $no++ }}</td>
                                    <td class="border px-4 py-2">{{ $holiday->date }}</td>
                                    <td class="border px-4 py-2">{{ $holiday->description }}</td>
                                    <td class="border px-4 py-2">
                                            <x-jet-button wire:click="edit( {{ $holiday->id}})" class="bg-orange-500 hover:bg-orange-700 m-1 w-20">
                                                Edit
                                            </x-jet-button>
                                    </td>
                                </tr>
